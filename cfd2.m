...
switch BCno
    case {2} % PDE BC for American Min Call
        ...
        Ab = kron(Jx0,Iy) + kron(Ix,Jy0) + kron(Jx,Jy);

        Ad = spdiag(coefs(:,1),m) * ...
                (kron(Ixn,Iyn)-kron(Jxn,Jyn)) + ... %u
            spdiag(coefs(:,2),m) * kron(A1x,Iyn) + ... %ux
            spdiag(coefs(:,3),m) * kron(A2x,Iyn) + ... %uxx
            spdiag(coefs(:,4),m) * kron(Ixn,A1y) + ... %uy
            spdiag(coefs(:,5),m) * kron(Ixn,A2y) + ... %uyy
        ...
        Am.Ix = Ix;
        Am.Iy = Iy;
        Am.Im = kron(Ixn,Iyn)-kron(Jxn,Jyn);
        Am.A1x = A1x;
        Am.A1y = A1y;
        Am.A2x = A2x;
        Am.A2y = A2y;
